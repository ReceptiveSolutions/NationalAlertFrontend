...
const MumbaiPage = ({ onArticleStore, navigate }) => {
  ...
  const getFallbackImage = (index = 0) => {
    ...
    return `https://source.unsplash.com/800x500/?${mumbaiImages[index % mumbaiImages.length]}`;
  };
  ...
  const getButtonText = (article, isExpanded) => {
    if (hasMoreThanMaxWords(article.description)) {
  ...
  const fetchMumbaiNews = async (attempt = 0) => {
    try {
      setIsLoading(true);
      setError(null);
      
      const response = await fetch(`${BASE_URL}/api/news/category/last24h?category=mumbai`);
      ...
      if (Array.isArray(articles) && articles.length > 0) {
        const processedArticles = articles.map((item, index) => {
          ...
          // if (content.includes('traffic') || content.includes('transport') || content.includes('railway')) {
          //   subcategory = 'Transport';
          // } else if (content.includes('weather') || content.includes('rain') || content.includes('monsoon')) {
          //   subcategory = 'Weather';
          // } else if (content.includes('bollywood') || content.includes('film') || content.includes('celebrity')) {
          //   subcategory = 'Entertainment';
          // } else if (content.includes('crime') || content.includes('police')) {
          //   subcategory = 'Crime';
          // } else if (content.includes('politics') || content.includes('election') || content.includes('government')) {
          ...
          return {
            id: item.article_id || item.id || `mumbai_${index}`,
            title: item.title || 'Mumbai News Update',
            description: item.description || item.summary || item.content || 'No Discription Available' ,
            summary: item.description || item.summary || item.content || 'No Discription Available',
      ...
      console.error(`Failed to fetch Mumbai news (attempt ${attempt + 1}):`, error);
      
      if (attempt < 2) {
        const delay = Math.pow(2, attempt) * 1000;
        setTimeout(() => {
          fetchMumbaiNews(attempt + 1);
        }, delay);
      } else {
        setError('Failed to load Mumbai news. Please try again later.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleArticleClick = (article) => {
    ...
    if (wordCount > maxWords) {
      const completeArticle = {
        ...article,
        id: article.id,
        title: article.title || 'Mumbai News Update',
        ...
        author: article.author || 'Mumbai Reporter',
        category: article.category || 'Mumbai',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-7xl mx-auto px-6 py-20">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-semibold text-gray-800 mb-2">Loading Mumbai News</h2>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center">
            ...
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">
              Stay updated with breaking news and trending stories from Mumbai
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
                ...
                >
                  ...
                  <div className="relative overflow-hidden">
                    ...
                    {/* <div className="absolute top-4 left-4">
                      {article.isHot ? (
                        <span className="bg-red-500 text-white px-3 py-1 text-xs font-medium rounded-full flex items-center">
          ...
          {/* {articles.length > 12 && (
            <div className="text-center mt-16">
              <button
                onClick={() => fetchMumbaiNews()}
                className="group border-2 border-gray-200 hover:border-gray-300 text-gray-700 hover:text-gray-900 px-8 py-4 rounded-xl font-medium transition-all duration-200 hover:bg-gray-50 flex items-center mx-auto"
              >
